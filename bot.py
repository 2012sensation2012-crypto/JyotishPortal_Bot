...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "<b>Выберите режим:</b>\n"
        "• <code>1</code> — будущие порталы (только дата + тип)\n"
        "• <code>2</code> — анализ по дате (полный отчёт)\n"
        "• <code>3</code> — прошлые порталы (только дата + тип)",
        parse_mode="HTML"
    )
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "📖 <b>Инструкция</b>\n\n"
        "• <code>1</code> → год → список дат и типов порталов\n"
        "• <code>2</code> → дата, место → полный анализ\n"
        "• <code>3</code> → год → список дат и типов порталов",
        parse_mode="HTML"
    )
...
async def handle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "1":
        await update.message.reply_text("⏳ Отправьте год (например, <code>2026</code>)", parse_mode="HTML")
    elif text == "3":
        await update.message.reply_text("⏳ Отправьте год (например, <code>1947</code>)", parse_mode="HTML")
    elif text == "2":
        await update.message.reply_text(
            "🔍 Пример:\n<code>5 июля 1947, Roswell, USA</code>", parse_mode="HTML"
        )
