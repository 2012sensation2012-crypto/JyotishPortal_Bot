...
# Состояния бота
(STATE_START, STATE_SELECT_CITY, STATE_SELECT_TYPE, STATE_ENTER_YEAR) = range(4)
...
def get_region_code(lat, lon):
    ...
    best_match = "MSK1"  # Код по умолчанию
...
def is_night(lat, lon, dt):
    ...
    try:
        ...
        # Создаем LocationInfo без elevation (новые версии astral)
        city = LocationInfo("", "", tz_str, lat, lon)
        
        # Получаем время восхода и заката с elevation=0
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Создаем кнопки с российскими городами (первые 25)
    keyboard = []
...
async def select_city(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Проверяем, что выбран город из списка
...
async def enter_year(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        # Начинаем анализ
        await update.message.reply_text(
            f"⏳ Начинаю анализ {year} года для {city}...\n\n"
        ...
        for month in range(1, 13):
            for day in range(1, 32):
                try:
                    ...
                    event_type, _ = get_event_analysis(0, 0, dt)  # Координаты будут определены в get_event_analysis
                    # Здесь нужно реализовать получение координат города
