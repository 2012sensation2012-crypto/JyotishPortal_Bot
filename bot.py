...
def get_kp_index(date):
    ...
    try:
        ...
        response = requests.get(
            "https://services.swpc.noaa.gov/products/noaa-3-day-forecast.json",
            timeout=10
        )
        ...
        # Форматируем дату как "20230625"
        date_str = date.strftime("%Y%m%d")
        
        # Поиск данных для нужной даты
        for entry in data[1:]:  # Пропускаем заголовок
            if len(entry) >= 2 and entry[0].startswith(date_str):
                try:
                    kp = float(entry[1])
        ...
        if date < datetime.datetime.now().date():
            ...
            for entry in data[1:]:
                if len(entry) >= 2 and entry[0].startswith(yesterday_str):
                    try:
                        kp = float(entry[1])
                        kp_cache[date] = (kp, current_time)
                        return kp
...
def is_night(lat, lon, dt):
    ...
    try:
        ...
        # Создаем LocationInfo с высотой 0 метров (параметр elevation теперь в LocationInfo)
