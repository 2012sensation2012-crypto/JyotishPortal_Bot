...
def classify_event_cached(lat, lon, dt):
    """Кэшированная версия classify_event для ускорения повторных расчётов"""
...
def classify_event(lat, lon, dt):
    """Основная функция классификации — использует кэш"""
...
async def manual_search(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        # === ИСПРАВЛЕНО: одна переменная, не две ===
        result = classify_event(lat, lon, dt)
