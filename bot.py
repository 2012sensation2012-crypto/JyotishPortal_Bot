...
# Указываем путь к папке с эфемеридами относительно текущего файла bot.py
# Если bot.py и папка ephemeris находятся в корне репозитория, то путь такой:
# Используем os.getcwd() — текущую директорию, откуда запущен скрипт
# Render.com может запускать скрипт из другой директории, поэтому ./ephemeris может не сработать.
# Лучше использовать os.path.dirname(__file__), чтобы путь был относительно bot.py
