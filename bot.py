...
# === НОВАЯ КОМАНДА: УСТАНОВКА МЕСТА АНАЛИЗА ===
async def set_location(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        text = update.message.text.split(maxsplit=1)[1].strip()
        ...
        await update.message.reply_text(
            f"📍 Точка анализа установлена: {lat:.4f}, {lon:.4f} ({country})\n"
            f"Теперь массовые поиски будут проводиться для этой точки.",
            parse_mode="HTML"
        )
    except Exception as e:
        await update.message.reply_text(
            "❌ Ошибка установки координат\n\n"
            "Используйте формат:\n"
            "/setlocation 33.3943, -104.5230\n"
            "Или:\n"
            "/setlocation Roswell, USA",
            parse_mode="HTML"
        )
...
async def reset_location(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "📍 Точка анализа сброшена\n"
        "Теперь массовые поиски будут проводиться для Мемфиса, США",
        parse_mode="HTML"
    )
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "• <code>/setlocation</code> — установить точку анализа\n"
        "• <code>/resetlocation</code> — сбросить точку анализа\n\n"
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "3️⃣ Установите точку анализа:\n"
        "   → <code>/setlocation 33.3943, -104.5230</code>\n"
        "   → Все массовые поиски будут проводиться для этой точки",
        parse_mode="HTML"
    )
...
if __name__ == "__main__":
    ...
    app.add_handler(CommandHandler("resetlocation", reset_location))
