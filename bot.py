...
# Используем путь относительно файла bot.py — работает даже если запускается из другой директории
...
async def handle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if len(parts) == 3:
            ...
            dt = datetime.datetime(year, month, day, 12, tzinfo=pytz.UTC)  # явно указываем UTC
        else:
...
if __name__ == "__main__":
    ...
    # Для Render.com: бот должен работать вечно, даже если нет HTTP-сервера
    try:
        print("🚀 Запуск бота...")
        app.run_polling()
        ...
        # На Render процесс не должен завершаться — ждём, чтобы Render не перезапускал
        import time
        while True:
            time.sleep(60)