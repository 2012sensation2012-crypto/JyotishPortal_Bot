...
def calculate_astrology(lat, lon, dt):
    ...
    # Рассчитываем дома по системе Кришнамурти (KP)
...
def get_houses_kp(lat, lon, jd):
    ...
    Система Кришнамурти использует систему домов Коши (Koch) 
    с дополнительными подразделами (Vargas) для точных расчётов
    """
    try:
        # Используем систему домов Коши (b'K'), ближайшую к системе Кришнамурти
        house_cusps, ascmc = swe.houses(jd, lat, lon, b'K')
        
        # Возвращаем только первые 12 куспидов (домов)
        return house_cusps[:12]
    except Exception as e:
        # Если основной метод не сработал, используем резервный расчёт
...
def get_dasha_period(dt, lat, lon):
    ...
    В системе Кришнамурти обычно используется Вимшоттари даша
    """
    # Определяем месяц
    month = dt.month % 9
    ...
    return dasha_planets[month], dasha_durations[month]